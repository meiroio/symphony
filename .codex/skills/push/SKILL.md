...
2. Run local validation (`cd bun && bun run typecheck && bun test`) before pushing.
...
7. Validate PR body with `cd bun && bun run check:pr-body` and fix all reported issues.
...
(cd bun && bun run typecheck && bun test)
...
(cd bun && bun run check:pr-body --file "$tmp_pr_body")
rm -f "$tmp_pr_body"
