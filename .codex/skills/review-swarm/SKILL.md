...
- in Codex, prefer `spawn_agent` with `agent_type: explorer`; if that is unavailable, use `default`
- never use `worker` for this skill because workers are intended for changes, not read-only review
...
Recommended sub-agent type: `explorer`
